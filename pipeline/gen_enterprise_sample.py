#!/usr/bin/env python3
"""
Sample data for the ENTERPRISE / ESTABLISHMENT DATA Coverage Index (illustrative,
full country roster).

Three criteria (coverage + frequency + recency), no pending. Self-contained
scoring that mirrors fetch_and_rank's formulas. Writes to ../web/data so the site
shows data immediately; replace with real ILOSTAT figures via:

    python fetch_and_rank.py --config config_enterprise --out ../web/data

    python gen_enterprise_sample.py     # writes ../web/data/rankings.(js|json)
"""
...
import config_enterprise as CE
...
# Illustrative establishment / enterprise / register source names.
ENTERPRISE_SOURCES = [
    "Establishment survey", "Enterprise survey", "Economic census",
    "Labour cost survey", "Occupational wage survey",
    "Social security records", "Business register",
]
...
def score(maturity):
    ...
    for ind in CE.KEY_INDICATORS:
        ...
        win = CE.SCORING["coverage"]["coverage_window_years"]
    ...
    pts = CE.SCORING["frequency"]["periodicity_points"]
    reg_win = CE.SCORING["frequency"]["regularity_window_years"]
...
def main():
    ...
    for iso, name, region, maturity in COUNTRIES:
        ...
        if latest:
            k = 2 if maturity >= 0.7 else 1
            src_names = random.sample(ENTERPRISE_SOURCES, k=min(k, len(ENTERPRISE_SOURCES)))
        ...
        records.append({
            ...
            "n_covered": n_cov, "n_total": len(CE.KEY_INDICATORS),
            "sources": sources,
            "indicators": {c["code"]: grid[c["code"]] for c in CE.KEY_INDICATORS},
        })

    payload = {
        "generated": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "current_year": CURRENT_YEAR,
        "index_name": CE.INDEX_NAME,
        "source": "SAMPLE DATA (illustrative) — replace via fetch_and_rank.py --config config_enterprise",
        "is_sample": True,
        "default_weights": CE.SCORING["weights"],
        "scoring_params": {k: v for k, v in CE.SCORING.items() if k != "weights"},
        "indicators": [
            {"code": i["code"], "label": i["label"], "tier": i["tier"], "weight": i["weight"]}
            for i in CE.KEY_INDICATORS
        ],
        "countries": records,
    }
