...
What it does
------------
1. Reads the indicator set, source filter and scoring rules from a config module
2. Downloads the ILOSTAT indicator table of contents (to validate dataset ids)
3. For every key indicator, downloads the relevant CSV(s) from the rplumber API
4. Keeps only observations whose source label passes the index's source filter
...
The website reads rankings.js. Re-run this whenever you change the config.

Usage
-----
    pip install -r requirements.txt
    python fetch_and_rank.py                                  # LFS (config.py)
    python fetch_and_rank.py --config config_enterprise --out ../web/data
    python fetch_and_rank.py --limit 3                        # quick test
          ...
          for period in ind["periodicities"]:
            ...
            try:
                ...
                for row in rows:
                    ...
                    n += 1
                print(f"      kept {n} qualifying observations")
...
def main():
    ...
    ap.add_argument("--config", default="config",
                    help="config module: 'config' (LFS), 'config_census', 'config_admin', 'config_enterprise'")
